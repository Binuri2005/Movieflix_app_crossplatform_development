...
import 'package:movie_app_example/models/tv_show.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  late Future<List<Movie>> kidsShowList;
  late Future<List<Movie>> highestGrossingMoviesList;
  ...
  void initState() {
    ...
    searchList = ApiService().getSearchMovie('');
    kidsShowList = ApiService().getKidsMovies();
    highestGrossingMoviesList = ApiService().highestGrossing();
  ...
  Widget build(BuildContext context) {
    ...
    return StreamProvider<List<Movie>>.value(
      value: DatabaseService(uid: 'ZDkzQnPeiBVeSCOLsDcvsmQqLpT2').moviesFromDoc,
      initialData: [],
      child: Scaffold(
        appBar: AppBar(
          actions: [
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const sizeHeight(),
                textTitleInHome('Children friendly', 25),
                const sizeHeight(),
                SizedBox(
                  child: FutureBuilder(
                    future: kidsShowList,
                ...
                const sizeHeight(),
                textTitleInHome('highest-grossing movies', 25),
                const sizeHeight(),
                SizedBox(
                  child: FutureBuilder(
                    future: highestGrossingMoviesList,
                    builder: (context, snapshot) {
                      if (snapshot.hasError) {
                        return Center(
                          child: Text(snapshot.error.toString()),
                            ...
                            suffixIcon: IconButton(
                              onPressed: () async {
                                setState(() {
                                  ...
                                  searchList =
                                      ApiService().getSearchMovie(query);
                                });
                              },
                              icon: const Icon(Icons.search),
                            ),
